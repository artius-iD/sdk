...
- `verifyEnrolledAccount()` checks with the service that this device's enrolled account still exists. It returns `.active`, `.inactive` (the service has no active account for the device, so it is safe to start enrollment again) or `.unreachable` (the check could not complete; keep the current state).
